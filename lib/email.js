...
export async function notifyNewOrder(order) {
  ...
  const accion = esOnline
    ...
    : esAddi
    ? "Genera el link de pago en tu panel de Addi con los datos del titular de abajo, y envíaselo por WhatsApp al celular del titular."
  ...
  const html = `
    <h2>Nuevo pedido #${formatOrderNumber(order.numero)}</h2>
    <p><b>Método de pago:</b> ${order.metodo_pago}</p>
    <p><b>Total:</b> ${formatCOP(order.total)}</p>
    <p><b>Cliente:</b> ${order.cliente_nombre}${order.cliente_cedula ? " — CC " + order.cliente_cedula : ""} — ${order.cliente_celular}</p>
    ...
    ${esAddi ? `
    <p style="margin-top:16px; padding:12px; background:#fff3cd; border:1px solid #ffe69c;">
      <b>Titular del cupo Addi:</b> ${order.addi_nombre}<br/>
      <b>Cédula:</b> ${order.addi_cedula}<br/>
      <b>Celular del titular:</b> ${order.addi_celular}
    </p>` : ""}
  ...
  return sendEmail({ subject: `🛍️ Nuevo pedido #${formatOrderNumber(order.numero)} — ${formatCOP(order.total)}`, html });
}
...
export async function notifyPaymentUpdate(order, estadoPago) {
  ...
  const html = `
    <h2>${aprobado ? "✅ Pago confirmado" : "❌ Pago rechazado"} — Pedido #${formatOrderNumber(order.numero)}</h2>
    <p><b>Total:</b> ${formatCOP(order.total)}</p>
    <p><b>Cliente:</b> ${order.cliente_nombre}${order.cliente_cedula ? " — CC " + order.cliente_cedula : ""} — ${order.cliente_celular}</p>
  ...
  `;
  return sendEmail({
    subject: `${aprobado ? "✅" : "❌"} Pedido #${formatOrderNumber(order.numero)} — ${aprobado ? "Pago confirmado" : "Pago rechazado"}`,
    html,
  });
}
