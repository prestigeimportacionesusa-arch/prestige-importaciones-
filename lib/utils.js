...
export const COMBO_PRICE = 409000;

// Recibe las líneas del carrito ya calculadas (con precio final normal por
// unidad) y agrupa en parejas las unidades de productos marcados para el
// combo. Cada pareja se cobra $409.000 en total — o su precio normal si ya
// costaría menos que eso (nunca sale más caro que comprarlos sueltos).
export function computeComboDiscount(lines) {
  const units = [];
  lines.forEach((line) => {
    if (line.combo) {
      for (let i = 0; i < line.qty; i++) units.push(line.precio);
    }
  });
  ...
  for (let i = 0; i + 1 < units.length; i += 2) {
    ...
    const charge = Math.min(sum, COMBO_PRICE);
    discount += sum - charge;
    pairs += 1;
  }
  const leftover = units.length % 2 === 1;
  return { discount, pairs, leftover };
}

// Pixel diminuto y oscuro, usado como "placeholder" mientras carga la foto
// real de un producto — evita ver una caja negra vacía o un salto brusco
// cuando la imagen termina de descargar (sobre todo en conexiones lentas).
export const BLUR_PLACEHOLDER = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAIAAABLbSncAAAAFElEQVR4nGMUERZjwAaYsIoOWgkANjQATYgpdMMAAAAASUVORK5CYII=";

// Los pedidos se numeran de forma secuencial en la base de datos (100, 101,
// 102...), pero se muestran siempre con 4 dígitos (0100, 0101...) para que
// se vean parejos.
