import { formatCOP, formatOrderNumber } from "./utils";
...
export function waOrderMessage(order) {
  const lines = [
    `Hola, confirmo mi pedido *#${formatOrderNumber(order.numero)}*:`,
    "",
    ...
    ...(order.cliente_cedula ? [`Cédula: ${order.cliente_cedula}`] : []),
    ...
    ...(order.addi_nombre ? [
      "",
      `Titular del cupo Addi: ${order.addi_nombre}`,
      `Cédula: ${order.addi_cedula}`,
      `Celular del titular: ${order.addi_celular}`,
    ] : []),
