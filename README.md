...
6. Ve a **Project Settings → API** y copia:
   ...
   * `Project URL` → esto es tu `NEXT\_PUBLIC\_SUPABASE\_URL`
   * `anon public key` → esto es tu `NEXT\_PUBLIC\_SUPABASE\_ANON\_KEY`
   * `service\_role secret key` → esto es tu `SUPABASE\_SERVICE\_ROLE\_KEY`
...
`.env.local` (la `service\_role` solo hace falta si vas a correr el seed
desde Vercel también; para producción normal solo se usan las dos
`NEXT\_PUBLIC\_\*` en el navegador/servidor).
